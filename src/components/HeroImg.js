...
useEffect(() => {
  setInterval(() => {
    ...
    if (state.currentImgLink ===state.arrayOfImages[state.imgArrCounter]) {
      ...
      document.querySelector('.heroImageContainer').style.backgroundColor="#f8c009"
    } else {
  document.querySelector('.heroImageContainer').style.backgroundColor="rgb(251 238 222)"
      setState({...state,
  ...
  const handleToggleClick=(e)=>{
    ...
    return (
      <div
        ...
        style={{ backgroundImage: `url(${state.currentImgLink})`,backgroundSize:"contain",backgroundPosition:"center",backgroundColor:"rgb(251 238 222)"}}
      >
        <div className="buttonsArea">
          <p> Welcome to shoes store</p>
          <h1 style={{color:"white"}}> you can get a diccount by using <b style={{color:"gold"}}>"OCA2022"</b>  coupon</h1>
          <button className="shopNowBtn"><Link to="/shop">Shop now</Link> </button>
